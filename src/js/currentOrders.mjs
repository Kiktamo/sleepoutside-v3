...
async function createPageList(currentPage, numPages) {
  ...
  // Next pages
  for (let i = currentPage + 1; i <= Math.min(numPages - 1, currentPage + 4); i++) {
...
function orderCardTemplate(order) {
  ...
  <p class="order__date"><b>Order Date:</b> ${new Date(order.orderDate).toDateString()}</p>
  <p class="order__total"><b>Order Total:</b> $${Number(order.orderTotal).toFixed(2)}</p><ul class="order-items">`;
